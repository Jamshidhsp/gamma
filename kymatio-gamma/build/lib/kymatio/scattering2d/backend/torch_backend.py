...
from ...backend.torch_backend import _is_complex, cdgmm, type_checks, Modulus, concatenate, log, concat_gamma, concat_gamma2
...
if version.parse(torch.__version__) >= version.parse('1.8'):
    ...
    fft1 = FFT(lambda x: torch.view_as_real(torch.fft.fft2(torch.view_as_complex(x))),
          lambda x: torch.view_as_real(torch.fft.ifft2(torch.view_as_complex(x))),
          lambda x: torch.fft.ifft2(torch.view_as_complex(x)).real,
          type_checks)
else:
    fft1 = FFT(lambda x: torch.fft(x, 2, normalized=False),
              lambda x: torch.ifft(x, 2, normalized=False),
              lambda x: torch.irfft(x, 2, normalized=False, onesided=False),
              type_checks)


backend = namedtuple('backend', ['name', 'cdgmm', 'modulus', 'subsample_fourier', 'fft', 'Pad', 'unpad', 'concatenate', 'log', 'concat_gamma', 'concat_gamma2'])
backend.name = 'torch'
backend.cdgmm = cdgmm
...
backend.log = log
backend.concat_gamma = concat_gamma
backend.concat_gamma2 = concat_gamma2
